        ...
        struct d3d12_device *device, const D3D12_ROOT_SIGNATURE_DESC2 *desc)
{
    ...
    /* [NULL-SRV-FALLBACK] Default to the "no fallback set" sentinel; the legacy
     * path below may append a real set when VKD3D_CONFIG_FLAG_RELAXED_UNMAPPED_SRV
     * is on. Consumers key off UINT32_MAX, so this must be initialized on every
     * path (memset-0 would alias a valid set index on the descriptor-heap path). */
        ...
        const struct d3d12_pipeline_state_desc *desc, struct d3d12_pipeline_state **state)
{
    ...
    if (VKD3D_CONFIG_FLAG_IS_SET(LAZY_PSO_COMPILE) &&
